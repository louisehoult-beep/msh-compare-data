...
def _repo_rel(path):
    """Path relative to the repo root, for anything written into the registry.

    Builds now happen in a throwaway clone under ~/.claude/msh-work/ (08/09/2026),
    so an absolute path here records whichever clone happened to run last and
    changes on every build - noise in a generated file that is reviewed by diff.
    Falls back to the absolute path if the file somehow sits outside the repo.
    """
    try:
...
def build(quiet=False):
    ...
    out = {
        ...
        "_sources": {"seed": _repo_rel(SEED), "overlay": _repo_rel(OVERLAY)},
        ...
        # Sorted by key, not built in dict-insertion order: the variants are
        # held in sets, whose iteration order changes with PYTHONHASHSEED, so
        # an unsorted index reshuffles on every build and a two-alias change
        # reads as a 10,660-line diff (^o419, 11/09/2026). Nothing here may be
        # left in insertion order.
        "index": {
            "exact": {k: sorted(v) for k, v in sorted(exact.items())},
            "normalised": {k: sorted(v) for k, v in sorted(normal.items())},
            "stripped": {k: sorted(v) for k, v in sorted(stripped.items())},
        },
    }
